...
for count,file_ in enumerate(allFiles):
    ...
    plt.plot(x,oneIt[:,0],label=file_)
...
plt.ylim(-0.1,1.1)
plt.xlim(-0.1,)
plt.legend(fontsize=12.5,loc=3)
# plt.xlabel('Time [s] / '+str(devisionScale),fontsize=15,fontweight='bold')
plt.xlabel('Time [s] / ',fontsize=15,fontweight='bold')
# plt.ylabel('epsilon and alpha parameter',fontsize=15,fontweight='bold')
plt.ylabel('epsilon',fontsize=15,fontweight='bold')
