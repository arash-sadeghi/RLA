...
colors=['red',"Blue"]
f=open('resultsSun Oct  4 22_23_07 2020.npy','rb')
data=np.load(f)
f.close()
...
q1=np.zeros((1,np.shape(dataOld)[1]))[0]
q2=np.zeros((1,np.shape(dataOld)[1]))[0]
q3=np.zeros((1,np.shape(dataOld)[1]))[0]

for col in range(np.shape(dataOld)[1]):
    q1[col]=np.percentile(dataOld[:,col],25)
    q2[col]=np.percentile(dataOld[:,col],50)
    q3[col]=np.percentile(dataOld[:,col],75)
X=np.linspace(0,endTime,len(q1),True)
plt.plot(X,q2,color=colors[1],label='BEECLUST with old simulator')
plt.fill_between(X,q2,q1,color=colors[1],alpha=0.25)
plt.fill_between(X,q3,q2,color=colors[1],alpha=0.25)
